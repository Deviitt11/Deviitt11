<div align="center">
  <h2>Estudiante de CFGS</h2>

  <p>
      <a href="https://www.linkedin.com/in/davidcb11">
        <img src="./assets/linkedin.svg" />
      </a>
  </p>
  
</div>

<!-- # Hola 👋, me llamo David
## Estudiante de CFGS
Estoy cursando un Ciclo Formativo de Grado Superior en Desarrollo de Aplicaciones Multiplataforma. Para expandir mis conocimientos, a menudo realizo cursos online e intento buscar todo tipo de recursos que puedan serme de utilidad a la hora de seguir formándome y continuar aprendiendo. Por ello, he decidido que una gran manera de reflejar y monitorizar mi aprendizaje es creando este portfolio, donde iré colgando los diferentes programas y proyectos que realice.  -->

<!-- ## Habilidades y Experiencia
* ☕ Java
* 🐍 Python
* 📚 SQL
* 🌐 HTML -->

<!-- ## Actualmente
- 🔭 El desarrollo de esta página está en continuo proceso.
- 🌱 Estoy aprendiendo Java, Python, HTML, CSS, SQL y JavaScript, entre otros lenguajes. -->

<!-- Tarjetas de forma individual -->

<!-- Lenguajes más usados -->
<div align="center">

  <h2>Actualmente</h2>
  <ul>
    <li style="text-align: center;">🔭 El desarrollo de esta página está en continuo proceso.</li>
    <li style="text-align: center;">🌱 Estoy aprendiendo Java, Python, HTML, CSS, SQL y JavaScript, entre otros lenguajes.</li>
  </ul>

  <a href="https://github.com/Deviitt11/convoychat">
    <img height=200 align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Deviitt11&layout=compact&langs_count=6&card_width=320&theme=dark&title_color=ffa500&text_color=ffffff&icon_color=ffa500&border_color=ffa500" />
  </a>
</div>

<!-- Github stats -->
<!-- <a href="https://github.com/Deviitt11/github-readme-stats">
  <img height=200 align="center" src="https://github-readme-stats.vercel.app/api?username=Deviitt11&show_icons=true&theme=dark&title_color=ffa500&text_color=ffffff&icon_color=ffa500&border_color=ffa500&rank_icon=github" />
</a> -->

<!-- Contador de visitas -->
<!-- <p align="center"> 
  <br>
  Visualizaciones <br>
  <img src="https://profile-counter.glitch.me/Deviitt11/count.svg" />
</p> -->



